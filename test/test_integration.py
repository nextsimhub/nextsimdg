...
if len(hice.shape) != n_hice_dim:
    print(
        f"Error: variable {hice_name} has the incorrect number of dimensions, expected {n_hice_dim}, got {len(hice.shape)}."
    )
    sys.exit(3)
if hice.shape[0] != ny:
    print(
        f"Error: incorrect first dimension for variable {hice_name}, expected {ny}, got {hice.shape[0]}"
    )
    sys.exit(4)
if hice.shape[1] != nx:
    print(
        f"Error: incorrect second dimension for variable {hice_name}, expected {nx}, got {hice.shape[1]}"
    )
    sys.exit(5)
if hice.shape[2] != ndg:
    print(
        f"Error: incorrect number of DG components for variable {hice_name}, expected {ndg}, got {hice.shape[2]}"
    )
    sys.exit(6)
...
if np.any(hice_dg0 <= hice_min) or np.any(hice_dg0 > hice_max):
    print(
        f"Error: ice thickness at at least one sample point is outside the range 0 < h_ice < 10 m."
    )
    sys.exit(7)
...
if np.any(cice_dg0 <= cice_min) or np.any(cice_dg0 > cice_max):
    print(
        f"Error: ice concentration at at least one sample point is outside the range 0 ≤ c_ice ≤ 1."
    )
    sys.exit(9)
