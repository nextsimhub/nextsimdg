...
if __name__ == "__main__":
    ...
    init_base.cice[:, :] = topaz4_interpolate(element_lon, element_lat, source_file["siconc"][data_time.tm_mday, :, :].squeeze(),
                                              source_x, source_y, proj_string)
    init_base.hice[:, :] = topaz4_interpolate(element_lon, element_lat, source_file["sithick"][data_time.tm_mday, :, :].squeeze(),
                                              source_x, source_y, proj_string)

    # Snow thickness
    init_base.hsnow[:, :] = topaz4_interpolate(element_lon, element_lat, source_file["sisnthick"][data_time.tm_mday, :, :].squeeze(),
                                               source_x, source_y, proj_string)
    # SSS
    init_base.sss[:, :] = topaz4_interpolate(element_lon, element_lat, source_file["so"][data_time.tm_mday, :, :].squeeze(), source_x,
                                             source_y, proj_string)

    # SST
    init_base.sst[:, :] = topaz4_interpolate(element_lon, element_lat, source_file["thetao"][data_time.tm_mday, :, :].squeeze(),
                                             source_x, source_y, proj_string)
