...
def bilinear_missing(eyes, jays, data, missing):
    ...
    weighted_sum = (
        (1 - fj) * (1 - fi) * data[j, i] * dataplier[j, i]
...
if __name__ == "__main__":
    ...
    sst_data = topaz4_interpolate(
        element_lon,
        element_lat,
        source_file["temperature"][0, :, :].squeeze(),
        lat_array,
    )
    ...
    cice_data = topaz4_interpolate(
        element_lon, element_lat, source_file["fice"][0, :, :].squeeze(), lat_array
    )
    hice_data = topaz4_interpolate(
        element_lon, element_lat, source_file["hice"][0, :, :].squeeze(), lat_array
    )
    ...
    hsnow_data = topaz4_interpolate(
        element_lon, element_lat, source_file["hsnow"][0, :, :].squeeze(), lat_array
    )
    hsnow_data *= noice
    ...
    sss_data = topaz4_interpolate(
        element_lon, element_lat, source_file["salinity"][0, :, :].squeeze(), lat_array
    )
    sss[:, :] = sss_data

    # SST
    sst = ncFile.createVariable("sst", "f8", hfield_dims)
    sst_data = topaz4_interpolate(
        element_lon,
        element_lat,
        source_file["temperature"][0, :, :].squeeze(),
    ...
    # Ice starts at rest
    u = ncFile.createVariable("u", "f8", hfield_dims)
    u[:, :] = 0
