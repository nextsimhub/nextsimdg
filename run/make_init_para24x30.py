...
if __name__ == "__main__":
    ...
    ncFile = netCDF4.Dataset("init_para24x30.nc", "w", format="NETCDF4")
    ...
    cice = ncFile.createVariable("cice", "f8", dg_dims)
    hice = ncFile.createVariable("hice", "f8", dg_dims)
    cice[:, :, :] = 0.0
    ...
    # Snow thickness
    hsnow = ncFile.createVariable("hsnow", "f8", dg_dims)
    hsnow[:, :, :] = 0.0
    hsnow[:, :, 0] = 0.125

    # damage
    damage = ncFile.createVariable("damage", "f8", dg_dims)
    damage[:, :, :] = 0.0
    ...
    # Ice temperature
    tice = ncFile.createVariable("tsurf", "f8", field_dims)
    ...
    # Ice starts at rest
    u = ncFile.createVariable("u", "f8", dg_dims)
    u[:, :, :] = 0

    v = ncFile.createVariable("v", "f8", dg_dims)
    v[:, :, :] = 0
