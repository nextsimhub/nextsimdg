...
namespace Nextsim {
  ...
  // Slope limiter based on D. Kuzmin: A vertex-based hierarchical slope limiter for p-adaptive discontinuous Galerkin methods. Journal of Computational and Applied Mathematics 2010.
  ...
  public:
    SlopeLimiter<DG>(const ParametricMesh& _mesh) : mesh(_mesh)
    {
      if (DG>=3)
	{
    ...
    void InitMinMax(CGVector<1>& _minv,
		    CGVector<1>& _maxv,
		    const DGVector<DG>& phi, size_t comp=0)
    {
	...
	for (size_t cy = p; cy < mesh.ny; cy += 2) //!< loop over all cells of the mesh
	  {
	    ...
	    for (size_t cx = 0; cx < mesh.nx; ++cx, ++c, ++cgi) //!< loop over all cells of the mesh
	      {
...
#pragma omp parallel for
    ...
    void ComputeAlphas(DGVector<1>& alpha, const DGVector<DG>& phi,
		       const CGVector<1>& _min, const CGVector<1>& _max)
    {
      // relative indices of the four vertices in minV/maxV
      const size_t cgindices[4] = {0,1,mesh.nx+1,mesh.nx+2};
...
#pragma omp parallel for schedule(static)
      for (size_t c = 0; c< mesh.nelements; ++c)
	{
	  ...
	  for (size_t i=0;i<4;++i)
	      ...
	      if (dv<-1.e-8)
		{
		  ...
		  //		  std::cout << minV(cgi+cgindices[i]) << "\t" << midvalue << "\t" << vertexvalues[i] << std::endl;
    ...
    void ComputeAlphasX(DGVector<1>& alphax, const DGVector<DG>& phi,
			const CGVector<1>& _min, const CGVector<1>& _max)
    {
      // relative indices of the four vertices in minV/maxV
      const size_t cgindices[4] = {0,1,mesh.nx+1,mesh.nx+2};
...
#pragma omp parallel for schedule(static)
    ...
    void ComputeAlphasY(DGVector<1>& alphay, const DGVector<DG>& phi,
			const CGVector<1>& _min, const CGVector<1>& _max)
    {
      // relative indices of the four vertices in minV/maxV
      const size_t cgindices[4] = {0,1,mesh.nx+1,mesh.nx+2};
