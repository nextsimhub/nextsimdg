...
public:
    ...
    void AddStressTensorCell(const Mesh& mesh, const double scale, const size_t c, const size_t cx,
        const size_t cy, CGVector<1>& tx, CGVector<1>& ty, const CellVector<0>& S11,
        const CellVector<0>& S12, const CellVector<0>& S22) const
    {
        ...
        Eigen::Matrix<double, 4, 1> lup1 = scale
            * (DG0_CG1_dX * S11.row(c).transpose() / mesh.hx
                + DG0_CG1_dY * S12.row(c).transpose() / mesh.hy);
        tx(cg_i + 0) += lup1(0);
        tx(cg_i + 1) += lup1(1);
        ...
        Eigen::Matrix<double, 4, 1> lup2 = scale
            * (DG0_CG1_dX * S12.row(c).transpose() / mesh.hx
                + DG0_CG1_dY * S22.row(c).transpose() / mesh.hy);
        ty(cg_i + 0) += lup2(0);
        ty(cg_i + 1) += lup2(1);
    ...
    void InterpolateDGToCGCell(const Mesh& mesh, const size_t c, const size_t cx, const size_t cy,
        CGVector<1>& cg_A, const CellVector<0>& A) const
    {
    ...
    void InterpolateDGToCGCell(const Mesh& mesh, const size_t c, const size_t cx, const size_t cy,
        CGVector<2>& cg_A, const CellVector<0>& A) const
    {
