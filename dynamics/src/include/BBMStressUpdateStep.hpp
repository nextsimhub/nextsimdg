            ...
            const double deltaT) override
    {
        ...
        #pragma omp parallel for
          ...
          //! Evaluate values in Gauss points (3 point Gauss rule in 2d => 9 points)
                    const Eigen::Matrix<double, 1, nGauss * nGauss> hGauss = (h.row(i) * PSI<DGadvection, nGauss>).array().max(0.0).matrix();
                    ...
                    //! exp(-C(1-A))
                    const Eigen::Matrix<double, 1, nGauss * nGauss> expC = (params.compaction_param * (1.0 - aGauss.array())).exp().array();
                    ...
                    //! Eqn. 22
                    const Eigen::Matrix<double, 1, nGauss * nGauss>  cohesion = params.C_lab * scale_coef * hGauss.array();
                    //! Eqn. 30
                    const Eigen::Matrix<double, 1, nGauss * nGauss>  compr_strength = params.compr_strength * scale_coef * hGauss.array() ;
