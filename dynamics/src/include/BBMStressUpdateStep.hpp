...
#pragma omp parallel for
        for (size_t i = 0; i < smesh.nelements; ++i) {
            ...
            Eigen::Matrix<double, 1, nGauss * nGauss> s11Gauss
                = stress[I11].row(i) * PSI<DGstress, nGauss>;
            Eigen::Matrix<double, 1, nGauss * nGauss> s12Gauss
                = stress[I12].row(i) * PSI<DGstress, nGauss>;
            Eigen::Matrix<double, 1, nGauss * nGauss> s22Gauss
                = stress[I22].row(i) * PSI<DGstress, nGauss>;
