...
public:
    ...
    ////////////////////////////////////////////////// Output of CG1 - Vector (OLD MESH)
    static void write_cgvector(const std::string& fname, const CGVector<1>& v, const Mesh& mesh)
    {
        ...
        OUT << "# vtk DataFile Version 2.0" << std::endl
            ...
            << "DIMENSIONS " << mesh.nx + 1 << " " << mesh.ny + 1 << " 1" << std::endl
    ...
    ////////////////////////////////////////////////// Output of CG2 - Vector
    static void write_cgvector(const std::string& fname, const CGVector<2>& v, const Mesh& mesh)
    {
        // extract variable name
        std::string variableName
            = fname.substr(fname.find("/") + 1, fname.find_first_of(".") - fname.find("/") - 1);
        ...
        OUT << "# vtk DataFile Version 2.0" << std::endl
            ...
            << "ASCII" << std::endl
            << "DATASET STRUCTURED_POINTS" << std::endl
            << "DIMENSIONS " << 2 * mesh.nx + 1 << " " << 2 * mesh.ny + 1 << " 1" << std::endl
            << "ORIGIN 0 0 0" << std::endl
    ...
    ////////////////////////////////////////////////// Output of CG - Vector (SasipMesh)
    template <int CG>
    static void write_cgvector(const std::string& fname, const CGVector<CG>& v, const SasipMesh& smesh)
    {
        // extract variable name
        std::string variableName
            = fname.substr(fname.find("/") + 1, fname.find_first_of(".") - fname.find("/") - 1);
        ...
        // Structure Points
        OUT << "# vtk DataFile Version 2.0" << std::endl
            << "output generated by Nextsim (SasipMesh)" << std::endl
            << "ASCII" << std::endl
    ...
    template <int CG>
    static void write_cg_velocity(const std::string& fname, const CGVector<CG>& vx, const CGVector<CG>& vy, const SasipMesh& smesh)
    {
        ...
        if (CG == 1) {
            ...
            assert((smesh.nx + 1) * (smesh.ny + 1) == vx.rows());
            for (size_t i = 0; i < smesh.nnodes; ++i)
                OUT << smesh.vertices(i, 0) << " " << smesh.vertices(i, 1) << " 0" << std::endl;
    ...
    static void write_cellvector(
        const std::string& fname, const CellVector<DGdegree>& v, const Mesh& mesh)
    {
        std::ofstream OUT(fname.c_str());
        if (!OUT.is_open()) {
            std::cerr << "Failed to open '" << fname << "'." << std::endl;
    ...
    template <int DGdegree>
    static void write_cellvector(
        const std::string& fname, int n, const CellVector<DGdegree>& v, const Mesh& mesh)
    {
    ...
    template <int DG>
    static void write_dg(const std::string& fname, const CellVector<DG>& v, const Mesh& mesh)
    {
        // extract variable name
        std::string variableName
            = fname.substr(fname.find("/") + 1, fname.find_first_of(".") - fname.find("/") - 1);
        ...
        for (size_t iy = 0; iy < mesh.ny; ++iy)
            for (size_t ix = 0; ix < mesh.nx; ++ix) {
                ...
                OUT << mesh.hx * (ix + 1) << " " << mesh.hy * (iy + 1) << " 0" << std::endl;
                OUT << mesh.hx * ix << " " << mesh.hy * (iy + 1) << " 0" << std::endl;
            }
        OUT << "CELLS " << mesh.nx * mesh.ny << " " << 5 * mesh.nx * mesh.ny << std::endl;
        size_t ii = 0;
        for (size_t iy = 0; iy < mesh.ny; ++iy)
            for (size_t ix = 0; ix < mesh.nx; ++ix, ++ii)
    ...
    template <int DG>
    static void write_dg(const std::string& fname, const CellVector<DG>& v, const SasipMesh& smesh)
    {
        // extract variable name
        std::string variableName
            = fname.substr(fname.find("/") + 1, fname.find_first_of(".") - fname.find("/") - 1);
    ...
    static void write_dg(
        const std::string& fname, int n, const CellVector<DG>& v, const Mesh& mesh)
    {
        write_dg(compose_vtkname(fname, DG, n), v, mesh);
    }
    template <int DG>
    static void write_dg(
        const std::string& fname, int n, const CellVector<DG>& v, const SasipMesh& smesh)
    {
    ...
    static void write_cg(
        const std::string& fname, int n, const CGVector<CGdegree>& v, const Mesh& mesh)
    {
        write_cgvector(compose_vtkname(fname, CGdegree, n), v, mesh);
    }
    template <int CGdegree>
    static void write_cg(
        const std::string& fname, int n, const CGVector<CGdegree>& v, const SasipMesh& smesh)
    {
        write_cgvector(compose_vtkname(fname, CGdegree, n), v, smesh);
    }
    template <int CGdegree>
    static void write_cg_velocity(
        const std::string& fname, int n, const CGVector<CGdegree>& vx, const CGVector<CGdegree>& vy, const SasipMesh& smesh)
    {
