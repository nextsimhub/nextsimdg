...
#define S2A(Q) (Q == 1 ? 1 : (Q == 3 ? 3 : (Q == 6 ? 6 : (Q == 8 ? 6 : -1))))
    ...
    DGVector<S2A(DGs)> Delta(const ParametricMesh& smesh, const DGVector<DGs>& E11, const DGVector<DGs>& E12,
        const DGVector<DGs>& E22, const double DeltaMin)
    {
...
#undef NGP
    ...
    template <int DGs>
    DGVector<S2A(DGs)> Shear(const ParametricMesh& smesh, const DGVector<DGs>& E11, const DGVector<DGs>& E12,
        const DGVector<DGs>& E22)
    {
...
#pragma omp parallel for
    ...
    template <int DGstress>
    DGVector<DGstress> TensorInvII(const ParametricMesh& smesh, const DGVector<DGstress>& E11, const DGVector<DGstress>& E12,
        const DGVector<DGstress>& E22)
    {
        DGVector<DGstress> Invariant(smesh);
