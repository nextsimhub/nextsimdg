...
namespace Nextsim {
  ...
  template<> template <>
  void CGParametricMomentum<2>::ProjectCGToDG( CellVector<3>& dg, const CGVector<2>& cg)
  {
...
#pragma omp parallel for
  ...
  template <> template<>
  void CGParametricMomentum<2>::ProjectCG2VelocityToDG1Strain( CellVector<8>& E11, CellVector<8>& E12, CellVector<8>& E22)
  {
...
#pragma omp parallel for
  ...
  template <> template<>
  void CGParametricMomentum<2>::ProjectCG2VelocityToDG1Strain(const ParametricTransformation<2, 8>& ptrans,
							      CellVector<8>& E11,
							      CellVector<8>& E12, CellVector<8>& E22)
  {
    assert(static_cast<long int>((2 * smesh.nx + 1) * (2 * smesh.ny + 1)) == vx.rows());
    assert(static_cast<long int>((2 * smesh.nx + 1) * (2 * smesh.ny + 1)) == vy.rows());
    assert(static_cast<long int>(smesh.nx * smesh.ny) == E11.rows());
...
#pragma omp parallel for
  ...
  template <int CG> template<int DG>
...
#pragma omp parallel for schedule(static)
  ...
  // Sasip-Mesh Interface
  template <int CG> template<int DG>
  void CGParametricMomentum<CG>::AddStressTensor(const ParametricTransformation<CG, DG>& ptrans,
						 const double scale, CGVector<CG>& tx,
						 CGVector<CG>& ty, const CellVector<DG>& S11, const CellVector<DG>& S12,
						 const CellVector<DG>& S22) const
  {
    // parallelization in tripes
...
#pragma omp parallel for schedule(static)
	  ...
	  if (cy % 2 == p) {
	    size_t c = smesh.nx * cy;
	    for (size_t cx = 0; cx < smesh.nx; ++cx, ++c) //!< loop over all cells of the mesh
	      AddStressTensorCell(ptrans, scale, c, cx, cy, tx, ty, S11, S12, S22);
	  }
        }
  }
  ...
  template <>
  void CGParametricMomentum<1>::DirichletZero(CGVector<1>& v) 
  {
  ...
  template <>
  void CGParametricMomentum<2>::DirichletZero(CGVector<2>& v) 
  {
  ...
  template <int CG> template<int DG>
  void CGParametricMomentum<CG>::InterpolateDGToCG(
...
#pragma omp parallel for
  ...
  template<int CG> template<int DG, int DGstress>
  void CGParametricMomentum<CG>::mEVPIteration(const size_t NT_evp, const double alpha, const double beta,
					       double dt_adv,
...
#pragma omp parallel for
  ...
  template void CGParametricMomentum<2>::AddStressTensor(const ParametricTransformation<2, 8>& ptrans,
							 const double scale, CGVector<2>& tx,
							 CGVector<2>& ty, const CellVector<8>& S11, const CellVector<8>& S12,
							 const CellVector<8>& S22) const;
  template void CGParametricMomentum<2>::AddStressTensor(const double scale, CGVector<2>& tx,
							 CGVector<2>& ty, const CellVector<8>& S11, const CellVector<8>& S12,
							 const CellVector<8>& S22) const;

  // --------------------------------------------------

  template void CGParametricMomentum<2>::mEVPIteration(size_t NT_evp, double alpha, double beta,
						       double dt_adv,
  ...
  template void CGParametricMomentum<1>::InterpolateDGToCG(
							   CGVector<1>& cg_A, const CellVector<1>& A) const;
  template void CGParametricMomentum<1>::InterpolateDGToCG(
							   CGVector<1>& cg_A, const CellVector<3>& A) const;
  template void CGParametricMomentum<2>::InterpolateDGToCG(
