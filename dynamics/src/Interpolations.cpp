...
#pragma omp parallel for
        for (size_t eid = 0; eid < smesh.nelements; ++eid) {
            ...
            const Eigen::Matrix<Nextsim::FloatType, 2, 4> gp
                = ParametricTools::getGaussPointsInElement<2>(smesh, eid);
            const Eigen::Matrix<Nextsim::FloatType, 4, 1> initial_in_gp(initial(gp(0, 0), gp(1, 0)),
                initial(gp(0, 1), gp(1, 1)), initial(gp(0, 2), gp(1, 2)),
            ...
            if (smesh.CoordinateSystem == SPHERICAL) {
                ...
                const double masscosJ
                    = (J.array() * GAUSSWEIGHTS<2>.array() * coslat.array()).sum();
                phi.row(eid) = 1. / masscosJ
                    * (((coslat.array() * ParametricTools::J<2>(smesh, eid).array()
                ...
                phi.row(eid) = SphericalTools::massMatrix<DG>(smesh, eid).inverse()
                    * ((PSI<DG, GAUSSPOINTS1D(DG)>.array().rowwise()
                           * (ParametricTools::J<GAUSSPOINTS1D(DG)>(smesh, eid).array()
                               * GAUSSWEIGHTS<GAUSSPOINTS1D(DG)>.array() * coslat.array()))
                            .matrix()
                        * initial_in_gp);
            } else if (smesh.CoordinateSystem == CARTESIAN) {
                phi.row(eid) = ParametricTools::massMatrix<DG>(smesh, eid).inverse()
                    * ((PSI<DG, GAUSSPOINTS1D(DG)>.array().rowwise()
                           * (ParametricTools::J<GAUSSPOINTS1D(DG)>(smesh, eid).array()
...
#pragma omp parallel for reduction(+ : error)
        for (size_t eid = 0; eid < smesh.nelements; ++eid) {
            ...
            if (smesh.CoordinateSystem == SPHERICAL) {
                ...
                error += (ParametricTools::J<GAUSSPOINTS1D(DG)>(smesh, eid).array()
                    * GAUSSWEIGHTS<GAUSSPOINTS1D(DG)>.array()
                    * (src_in_gauss.array() - initial_in_gp.array()).square())
                             .sum();
            else
...
#pragma omp parallel for reduction(+ : error)
        for (size_t row = 0; row < smesh.ny; ++row) {
            ...
            for (size_t col = 0; col < smesh.nx;
                 ++col, ++eid, cgi += CG) { // loop over all elements
                ...
                if (CG == 1) {
                    ...
                    local << src(cgi), src(cgi + 1), src(cgi + 2), src(cgi + cgshift),
                        src(cgi + 1 + cgshift), src(cgi + 2 + cgshift), src(cgi + 2 * cgshift),
                        src(cgi + 1 + 2 * cgshift), src(cgi + 2 + 2 * cgshift);
                } else
                    abort();
                ...
                if (smesh.CoordinateSystem == SPHERICAL) {
                    ...
                    error += (ParametricTools::J<3>(smesh, eid).array() * GAUSSWEIGHTS<3>.array()
                        * (src_in_gauss.array() - initial_in_gp.array()).square())
                                 .sum();
                else
                    abort();
