/*!
 * @file benchmark_compression_meb.cpp
 * @date 06 March 2023
...
public:
    ...
    {
        constexpr double twohours = 2. * 60.0 * 60.0;
        if (time < twohours) {
...
void run_benchmark(const std::string meshfile)
{
    ...
    std::string resultsdir = "Compression_" + std::to_string(CG) + "_" + std::to_string(DGadvection) + "_" + std::to_string(DGstress)
        + "__" + std::to_string(NX);
    ...
    double dt_adv = 30. ; //60.0; //!< Time step of advection problem
    //double dt_adv = ReferenceScale::L / smesh.nx / 1000. * 5;  //!< Stable time step
    size_t NT = RefScaleCompression::T / dt_adv + 1.e-4; //!< Number of Advections steps
    ...
    //! VTK output
    double T_vtk = .05 * 60.0 * 60.0; // every 4 hours
    ...
    for (size_t timestep = 1; timestep <= NT; ++timestep) {
        ...
        for (size_t mebstep = 0; mebstep < NT_meb; ++mebstep) {
            ...
            //momentum.BBMStep(Params, NT_meb, dt_adv, H, A, D);
...
int main()
{
    ...
    meshes.push_back("../ParametricMesh/compression_30x60.smesh");// 2km mesh
    //meshes.push_back("../ParametricMesh/compression_60x120.smesh");// 1km mesh
