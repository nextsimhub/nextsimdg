...
``DGField``, ``DGStressField``, and ``CGField``. When reading from file, the
field type will be deduced from its dimension. When writing to file, the field
type should be set using the ``setFieldType`` member function of the ``Xios``
handler class.
