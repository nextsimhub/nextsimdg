...
ModelArray& ModelArray::setData(const ModelArray& source, size_t sourceStartIndex, size_t targetStartIndex)
{
    size_t sourceMaxLength = source.size() - sourceStartIndex;
    size_t targetMaxLength = size() - targetStartIndex;
    size_t length = std::min(sourceMaxLength, targetMaxLength);
    m_data(Eigen::seqN(targetStartIndex, length), Eigen::all) = source.m_data(Eigen::seqN(sourceStartIndex, length),Eigen::all);

    return *this;
}

ModelArray& ModelArray::setData(const ModelArray& source, MultiDim sourceStartLoc, MultiDim targetStartLoc)
{
    return setData(source, source.indexFromLocation(sourceStartLoc), this->indexFromLocation(targetStartLoc));
}
