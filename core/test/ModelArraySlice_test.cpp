...
TEST_CASE("Test buffers")
{
    ...
    std::vector<FloatType> shortBuffer;
    ...
    std::vector<FloatType> vectorBuffer;
    ...
    // std::array<x1-x0>
    std::array<FloatType, x1 - x0> arrayBuffer;
...
TEST_CASE("Eigen copying")
{
    ...
    Eigen::Matrix<FloatType, Eigen::Dynamic, DG, Eigen::RowMajor> eig;
    ...
    for (size_t i = 0; i < nx + 2 * nWrap; ++i) {
        REQUIRE(eig(Indexer::indexer(eigDim, { i, testRow }), 0)
            == source((i + nx - nWrap) % nx, testRow));
    }
    ...
    Slice topRow1 { { {}, { ny - 1 } } };
    Slice topRow2 { { {}, { -1 } } };
    // The data target is the bottom row of the target array, excluding the first and last points in
    // x
    Slice bottomRowBlock1 { { { 1, -1 }, { 0 } } };
    SliceIter eig1BottomRowBlock(bottomRowBlock1, eig1Dim);

    // The target array
    Eigen::Matrix<FloatType, Eigen::Dynamic, DG, Eigen::RowMajor> eig1;
...
TEST_CASE("Eigen (ModelArray::DataType) buffers")
{
    ...
    REQUIRE(
        eaSrc(Indexer::indexer({ sliceNx, sliceNy }, { 2, 3 }), 4) == 4 + xMul * (2 + yMul * 3));
    ...
    REQUIRE(
        eaSink(Indexer::indexer({ sliceNx, sliceNy }, { 3, 5 }), 2) == 2 + xMul * (3 + yMul * 5));
}
...
TEST_CASE("Eigen (ModelArray::DataType) subscripted buffers")
{
    ...
    for (size_t j = 0; j < ny; ++j) {
        for (size_t i = 0; i < nx; ++i) {
            FloatType spatial = xMul * (i + yMul * j);
            for (size_t c = 0; c < DG; ++c) {
    ...
    perimeterBuffer(Eigen::seqN(0, nx), Eigen::all)
        = static_cast<ModelArray::DataType>(maSrc[bottom]);
    // right nx, ny points
    perimeterBuffer(Eigen::seqN(nx, ny), Eigen::all)
        = static_cast<ModelArray::DataType>(maSrc[right]);
    // top nx + ny, nx points
    perimeterBuffer(Eigen::seqN(nx + ny, nx), Eigen::all)
        = static_cast<ModelArray::DataType>(maSrc[top]);
    // left 2nx + ny, ny points
    perimeterBuffer(Eigen::seqN(2 * nx + ny, ny), Eigen::all)
        = static_cast<ModelArray::DataType>(maSrc[left]);
