...
TEST_CASE("Multidimensional indexing")
{
    ...
    while (!iter3748.isEnd()) {
        ...
        bool yStep = deltaIndex
            == (Indexer::indexer({ nx, ny }, { 3, 5 }) - Indexer::indexer({ nx, ny }, { 6, 4 }));
    ...
    Slice elements8d { { { 2, 6 }, { 4, 9 }, { 6, 12 }, { 8, 15 }, { 10, 18 }, { 12, 21 },
        { 14, 24 }, { 16, 27 } } };
...
end8d:
    REQUIRE_MESSAGE(message.empty(), "Position ", to_string(Indexer::deIndexer(ni, iter8d.index())),
        " is out of bounds ", to_string(elements8d));
    REQUIRE(count == expt);
    ...
    SliceIter::MultiDim shapeTarget
        = { static_cast<size_t>(ceil(dim[0] - xi, dx)), static_cast<size_t>(ceil(lenY, dy)) };
...
bool matchIndices(SliceIter& si, std::vector<size_t> compare, bool debug = false)
{
    ...
    while (!si.isEnd()) {
        if (debug)
            std::cout << si.index() << "?=" << compare[compareIndex] << " ";
        if (compareIndex == compare.size())
            return false;
        if (debug)
            std::cout << " √ compare.size";
        if (si.index() != compare[compareIndex])
            return false;
        ++si;
        ++compareIndex;
        if (debug)
...
TEST_CASE("Zero step") { REQUIRE_THROWS(makeZeroStep()); }
