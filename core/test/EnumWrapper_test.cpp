...
TEST_CASE("Configure a wrapped enum", "[EnumWrapper]")
{
    ...
    cfgText << "[option]" << std::endl << "colour = " << targetColourString << std::endl;

    boost::program_options::options_description opt("EnumWrapper test options");
    opt.add_options()(colourName.c_str(),
        boost::program_options::value<EnumWrap::EnumWrapper<Colour>>(),
        "What is you favourite colour?");
    boost::program_options::variables_map vm;
    boost::program_options::store(
        boost::program_options::parse_config_file(cfgText, opt, true), vm);
