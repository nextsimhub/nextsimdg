...
void ThermoIce0::calculateElement(size_t i, const TimestepTime& tst)
{
    ...
    double k_lSlab = kappa_s * Ice::kappa / (kappa_s * hiSlab + Ice::kappa * hsSlab) * gamma;
    ...
    // of ice or snow
    double meltingLimit = (hsSlab > 0) ? 0 : freezingPointIce;
    ...
    double nowSnow = hsSlab + (snowMeltRate - snowSublRate) * tst.step;
    ...
    if ( hiSlab > 0. )
        hsSlab += snowfall[i] * tst.step / Ice::rhoSnow;
    ...
    double iceDraught = (hiSlab * Ice::rho + hsSlab * Ice::rhoSnow) / Water::rhoOcean;

    if (doFlooding && iceDraught > hiSlab) {
        double snowDraught = iceDraught - hiSlab;
        snowToIce[i] = snowDraught;
        hsSlab -= snowDraught * Ice::rho / Ice::rhoSnow;
        hiSlab = iceDraught;
    } else {
        snowToIce[i] = 0;
    }
    ...
    if (hiSlab < IceMinima::h()) {
        if (deltaHi[i] < 0) {
            double scaling = oldHiSlab / deltaHi[i];
            topMelt[i] *= scaling;
            botMelt[i] *= scaling;
        }
        ...
        qio[i] += hiSlab * bulkLHFusionIce / tst.step + hsSlab * bulkLHFusionSnow / tst.step;

        // No ice, no snow and the surface temperature is the melting point of ice
        hice[i] = 0.;
        hsnow[i] = 0.;
        tice_i = celsius(Ice::Tm);
    }
