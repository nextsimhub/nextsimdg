...
void BenchmarkAtmosphere::update(const TimestepTime& tst)
{
    ...
    constexpr FloatType oneday = 24.0 * 60.0 * 60.0;
    // maximum wind velocity of the cyclone
    constexpr FloatType vMax = 30.0;

    // number of days elapsed since t0
    Duration elapsedTime = tst.start - t0;
    FloatType timeFraction = elapsedTime.seconds() / oneday;
    FloatType cycloneDuration = 5.; // days

    // cyclone parameters
    const FloatType A = 1e-5;
    const FloatType k = 1e-5; // m⁻¹
    const FloatType alpha = 72. / 180. * M_PI;
    const FloatType cosalpha = cos(alpha);
    const FloatType sinalpha = sin(alpha);

    // centre of the cyclone (metres)
    FloatType x0 = BenchmarkCoordinates::nx() * BenchmarkCoordinates::dx * 0.5
        * (1 + timeFraction / cycloneDuration);
    FloatType y0 = BenchmarkCoordinates::ny() * BenchmarkCoordinates::dy * 0.5
        * (1 + timeFraction / cycloneDuration);
    ...
    for (size_t j = 0; j < BenchmarkCoordinates::ny(); ++j) {
        for (size_t i = 0; i < BenchmarkCoordinates::nx(); ++i) {
            ...
            // FloatType scale = exp(1.0) / 100.0 * exp(-0.01e-3 * sqrt(SQR(x - cMx) + SQR(y -
            // cMy)))
            // * 1.e-3;
            FloatType scale = A * exp(-k * hypot(xPrime(i, j), yPrime(i, j)));
